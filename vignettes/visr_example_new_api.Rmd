...
```{r}
surv_eq <- "survival::Surv(time, status) ~ sex"
surv_eq <- "survival::Surv(AVAL, CNSR) ~ sex"


km_summ <- vr_kaplan_meier_summary(lung_cohort, surv_eq)
...
# hack a function to annotate the km summary 

```{r}
add_km_summary <- function(gg, ...){
  
  label1 <-
    paste0(
      km_summ[[1]]$strata,
      " median surv time = ",
      km_summ[[1]]$`median survival time`,
      " (",
      km_summ[[1]]$`0.95LCL`,
      ", ",
      km_summ[[1]]$`0.95UCL`,
